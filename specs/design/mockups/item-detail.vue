...
const isWorking = computed(
  () => status.value === 'pending' || status.value === 'extracting',
)
  ...
  <article class="app-detail" :data-theme="theme">
    <div class="app-detail__inner">
      ...
      <figure class="app-lead">
        <div class="app-lead__media" role="img"
      ...
      <div class="app-detail__tags">
        <span v-for="tag in tags" :key="tag" class="app-tag-chip app-tag-chip--removable">
          ...
          <button type="button" class="app-tag-chip__remove" :aria-label="`Remove tag ${tag}`"
            @click="removeTag(tag)">
        ...
        <form class="app-tag-add" @submit.prevent="addTag">
          <input v-model="draftTag" class="app-tag-add__input" type="text"
            placeholder="Add tag" aria-label="Add a tag" maxlength="24" />
          <button type="submit" class="app-tag-add__btn" :disabled="!draftTag.trim()"
            aria-label="Add tag">
      ...
      <div class="app-detail__controls">
        ...
        <div class="app-segment" role="group" aria-label="Read state">
          <button v-for="seg in READ_SEGMENTS" :key="seg.key" type="button"
            class="app-segment__btn" :aria-pressed="readState === seg.key"
        ...
        <button type="button" class="app-fav" :class="{ 'app-fav--on': favorite }"
          :aria-pressed="favorite" @click="toggleFavorite">
      ...
      <section class="app-fetch" :class="`app-fetch--${status}`" aria-live="polite">
        ...
        <div v-if="status === 'idle'" class="app-fetch__pitch">
          ...
          <button type="button" class="app-button app-button--primary app-fetch__cta"
            @click="fetchArticle">
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor"
              stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <div v-else-if="isWorking" class="app-fetch__working" role="status">
          <div class="app-fetch__working-head">
            <span class="app-status-badge"
              :class="status === 'pending' ? 'app-status-badge--pending' : 'app-status-badge--extracting'">
          ...
          <div class="app-reader-preview__actions">
            ...
            <a class="app-button app-button--ghost" :href="item.url" rel="noreferrer">
              ...
              <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor"
                stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                <path d="M14 5h5v5" /><path d="M19 5l-8 8" /><path d="M19 14v4a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1h4" />
              </svg>
            </a>
          </div>
        </div>
        ...
        <div v-else class="app-fetch__failed">
          ...
          <div class="app-fetch__failed-actions">
            ...
            <a class="app-button app-button--ghost" :href="item.url" rel="noreferrer">Open original</a>
          </div>
        </div>
      </section>
    </div>
...
.app-detail__inner > *:nth-child(1) { animation-delay: 0ms; }
.app-detail__inner > *:nth-child(2) { animation-delay: var(--dur-fast); }
.app-detail__inner > *:nth-child(3) { animation-delay: var(--dur-base); }
.app-detail__inner > *:nth-child(4) { animation-delay: calc(var(--dur-base) + var(--dur-fast)); }
.app-detail__inner > *:nth-child(5) { animation-delay: var(--dur-slow); }
.app-detail__inner > *:nth-child(6) { animation-delay: calc(var(--dur-slow) + var(--dur-fast)); }
.app-detail__inner > *:nth-child(7) { animation-delay: calc(var(--dur-slow) + var(--dur-base)); }
...
.app-lead {
  ...
  &__media {
    ...
    overflow: hidden;
    background:
      radial-gradient(120% 130% at 16% 10%, color-mix(in oklab, var(--brand-accent) 30%, transparent), transparent 55%),
      radial-gradient(120% 130% at 86% 92%, color-mix(in oklab, var(--brand-accent) 16%, transparent), transparent 60%),
...
@keyframes app-fetch-indeterminate {
  ...
  60% { transform: translateX(120%) scaleX(1); }
