    ...
    items.value = items.value.map((i) =>
      i.id === id
        ? {
            ...
            excerpt: 'Reader view ready — open to read the full article, highlight, and sync to your devices.',
          }
        : i,
    )
  }, 2400)
...
function onInput() {
  ...
  else if (looksValid.value) note.value = { kind: 'ok', text: 'Looks like a link — press Save to keep it.' }
  else note.value = { kind: 'hint', text: 'Keep typing a full address, e.g. https://example.com/article.' }
}
...
function onSave() {
  ...
  if (!URL_RE.test(raw)) {
    note.value = { kind: 'error', text: 'That doesn’t look like a link. Try a full address like https://example.com/article.' }
    return
  }
...
const mNote = ref<{ kind: NoteKind; text: string }>({ kind: 'hint', text: 'We’ll grab the title, a clean excerpt, and a reader view automatically.' })
const mInput = ref<HTMLInputElement | null>(null)

function openModal() {
  modalOpen.value = true
  mUrl.value = ''
  mNote.value = { kind: 'hint', text: 'We’ll grab the title, a clean excerpt, and a reader view automatically.' }
...
function onModalSave() {
  ...
  if (!URL_RE.test(raw)) {
    mNote.value = { kind: 'error', text: 'That doesn’t look like a link. Try a full address like https://example.com/article.' }
    return
  }
  mSaving.value = true
  window.setTimeout(() => {
  ...
  <div class="app-add-link" :data-theme="theme">
    <div class="app-add-link__inner">
      <header class="app-add-link__head">
        <div>
          ...
          <p class="app-add-link__subtitle">{{ items.length }} saved · paste anything worth reading later</p>
        </div>
        <!-- @app/ui: AppButton (opens the modal presentation) -->
        <button class="app-button app-button--ghost app-add-link__open" type="button" @click="openModal">
      ...
      <!-- ============================ 1 · inline add bar ============================ -->
      <!-- @app/ui: AppAddBar -->
      <form class="app-add-bar" :class="{ 'app-add-bar--saving': saving }" novalidate @submit.prevent="onSave">
        <div class="app-add-bar__field">
          <svg class="app-add-bar__icon" viewBox="0 0 24 24" width="18" height="18" fill="none"
        ...
        <!-- @app/ui: AppButton -->
        <button class="app-button app-add-bar__submit" type="submit" :aria-busy="saving || undefined" :disabled="saving">
          <span v-if="saving" class="app-spinner" aria-hidden="true" />
          <span>{{ saving ? 'Saving…' : 'Save' }}</span>
        </button>
        <p id="add-bar-note" class="app-add-bar__note" :class="`app-add-bar__note--${note.kind}`" :role="invalid ? 'alert' : 'status'">
    ...
    <Transition name="app-modal">
      ...
      <div v-if="modalOpen" class="app-modal" @keydown.esc="closeModal">
        ...
        <div class="app-modal__sheet" role="dialog" aria-modal="true" aria-labelledby="add-modal-title">
          <header class="app-modal__head">
            <h2 id="add-modal-title" class="app-modal__title">Save a link</h2>
            <button class="app-icon-btn app-modal__close" type="button" aria-label="Close" @click="closeModal">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor"
                stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          ...
          <form class="app-modal__body" novalidate @submit.prevent="onModalSave">
            <div class="app-field">
              ...
              <p id="modal-note" class="app-add-bar__note" :class="`app-add-bar__note--${mNote.kind}`" :role="mNote.kind === 'error' ? 'alert' : 'status'">
                <span v-if="mNote.kind !== 'hint'" class="app-add-bar__note-dot" aria-hidden="true" />
                {{ mNote.text }}
              </p>
            </div>

            <div class="app-modal__foot">
              <button class="app-button app-button--ghost" type="button" :disabled="mSaving" @click="closeModal">Cancel</button>
              <!-- @app/ui: AppButton -->
              <button class="app-button" type="submit" :aria-busy="mSaving || undefined" :disabled="mSaving">
...
.app-modal-leave-active .app-modal__sheet {
  transition: transform var(--dur-slow) var(--ease-decelerate), opacity var(--dur-base) var(--ease-standard);
}
...
.app-add-bar { animation: app-rise var(--dur-slow) var(--ease-decelerate) both; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .app-add-bar,
  .app-item-card--fresh { animation: none; }
  .app-button:active:not(:disabled) { transform: none; }
  .app-modal-enter-active .app-modal__sheet,
  .app-modal-leave-active .app-modal__sheet { transition: opacity var(--dur-base) var(--ease-standard); }
  .app-modal-enter-from .app-modal__sheet,
  .app-modal-leave-to .app-modal__sheet { transform: none; }
}
</style>
