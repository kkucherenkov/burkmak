<script setup lang="ts">
  ...
  const SEED: LibraryItem[] = [
    ...
    {
      id: 'i2',
      title: 'Self-hosting your read-it-later, end to end',
      site: 'arstechnica.com',
      url: 'https://arstechnica.com/x',
      savedAt: '5h ago',
      excerpt:
        'A walkthrough of running an extraction pipeline on a small box at home, from article fetch to a clean reader view, with sync to an e-reader.',
    ...
    {
      id: 'i3',
      title: 'How Kobo sideloading actually works',
      site: 'mobileread.com',
      url: 'https://mobileread.com/x',
      savedAt: '8h ago',
      excerpt:
        'The file formats, the folder the device watches, and the metadata that decides whether your saved articles show up as a tidy collection or a mess.',
      tags: ['kobo', 'sync'],
      status: 'extracting',
      readState: 'unread',
      starred: false,
    ...
    {
      id: 'i4',
      title: 'Exporting highlights to Obsidian without the cruft',
      site: 'obsidian.md',
      url: 'https://obsidian.md/x',
      savedAt: 'Yesterday',
      excerpt:
        'Markdown export looks simple until footnotes, callouts, and block references get involved. A small template keeps your notes clean on the way out.',
      tags: ['obsidian', 'notes'],
      status: 'ready',
      readState: 'unread',
      starred: false,
    ...
    {
      id: 'i5',
      title: 'A field guide to warm, low-glare dark modes',
      site: 'smashingmagazine.com',
      url: 'https://smashingmagazine.com/x',
      savedAt: '2d ago',
      excerpt:
        'Pure black is rarely the right answer for long reading. Warming the ink and the paper reduces glare without the muddy contrast of a flat grey theme.',
      tags: ['design', 'reading'],
      status: 'ready',
      readState: 'read',
      starred: true,
    ...
    {
      id: 'i6',
      title: 'Why your saved articles never finished downloading',
      site: 'github.blog',
      url: 'https://github.blog/x',
      savedAt: '3d ago',
      excerpt: '',
      tags: ['devops'],
      status: 'failed',
    ...
    {
      id: 'i7',
      title: 'Notes on building a calmer inbox for the things you mean to read',
      site: 'craigmod.com',
  ...
  const tags = computed(() => Array.from(new Set(items.value.flatMap((i) => i.tags))).sort());
  ...
  const empty = computed(() => {
    ...
    if (q) {
      return {
        title: 'No matches',
        text: `Nothing matches “${q}”. Try a different word or clear the search.`,
        cta: 'Clear search',
      };
    }
    if (activeTag.value !== 'all') {
      return {
        title: 'No items here',
        text: `Nothing tagged #${activeTag.value} in this view yet.`,
        cta: '',
      };
    }
    switch (filter.value) {
      case 'archived':
        return {
          ...
          text: 'Articles you archive rest here, out of the way but never gone.',
          cta: '',
        };
      case 'read':
        return {
          title: 'Nothing read yet',
          text: 'Finished articles move here so your unread list stays calm.',
          cta: '',
        };
      case 'starred':
        return {
          title: 'No favorites yet',
          text: 'Star an article to keep it close — favorites collect here.',
          cta: '',
        };
      default:
        return {
          title: 'Nothing here yet',
          text: 'Paste a link to start your library. burkmak fetches a clean reader view and keeps it for later.',
          cta: 'Paste a link',
        };
    }
  });
      ...
      items.value = items.value.map((i) =>
        i.id === id
          ? {
              ...i,
              status: 'ready',
              excerpt: 'Fetched on retry — the reader view is ready to open.',
            }
          : i,
      );
    }, 1100);
    ...
    <div class="app-library__inner">
      ...
      <div class="app-filter-bar" role="toolbar" aria-label="Filter library">
        <div class="app-filter-bar__seg" role="group" aria-label="Read state">
          ...
          >
            <svg
              v-if="seg.star"
              class="app-filter-bar__seg-icon"
              viewBox="0 0 24 24"
              width="15"
              height="15"
              fill="none"
      ...
      <div v-if="loading" class="app-list" aria-busy="true" aria-label="Loading saved items">
        ...
        <article
          v-for="n in 5"
          :key="n"
          class="app-item-card app-item-card--skeleton"
          aria-hidden="true"
        >
        ...
        >
          ...
          <div class="app-item-card__body">
            ...
            <p
              ...
              class="app-item-card__excerpt app-item-card__excerpt--failed"
            >
              <span>We couldn’t fetch this article.</span>
              <button type="button" class="app-item-card__retry" @click="retry(item.id)">
                Retry
              </button>
            </p>
          ...
          <div class="app-item-card__actions">
            ...
            >
              ...
              >
                <path
                  d="M12 3.5l2.6 5.4 5.9.8-4.3 4.1 1 5.9-5.2-2.8-5.2 2.8 1-5.9L4.5 9.7l5.9-.8z"
                />
              </svg>
            </button>
            <button
      ...
      <div v-else class="app-empty-state">
        <span class="app-empty-state__icon" aria-hidden="true">
          <svg
            ...
            stroke-linejoin="round"
          >
            <path d="M5 13l2-7h10l2 7v5a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1z" />
            <path d="M4 13h4l2 3h4l2-3h4" />
          </svg>
        </span>
        <h2 class="app-empty-state__title">{{ empty.title }}</h2>
        <p class="app-empty-state__text">{{ empty.text }}</p>
        <button v-if="empty.cta" type="button" class="app-empty-state__cta" @click="clearSearch">
          {{ empty.cta }}
        </button>
      </div>
    </div>
