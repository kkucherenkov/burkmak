...
- Every schema change is a committed Prisma migration (`prisma migrate dev`). No ad-hoc SQL on prod data.
- **`prisma db push` is prohibited** (even in dev). The DB — local, container, and CI — is provisioned only by applying committed migrations (`prisma migrate deploy`). `db push` drifts the DB away from the migration history and leaves no `_prisma_migrations` baseline. The dev container entrypoint runs `prisma migrate deploy`.
