...
- **i18n is mandatory**: no string literals in templates. Every visible string goes through `useI18n().t('namespace.key')` with keys defined in `apps/web/i18n/locales/{en,ru}.ts` global locale files (both `en` and `ru` keys required). No `<i18n>` SFC blocks — Vite 8 cannot build them. See `.claude/docs/i18n.md`.
